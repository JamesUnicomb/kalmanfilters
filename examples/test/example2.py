...
kf = kalmanfilters.Tracking(50.0)
...
for i in range(3):
    ax[i].plot(tacc, np.abs(accp[:,i] - acc[:,i]))
    ax[i].fill_between(
        tacc,
        np.zeros_like(tacc),
       2.0 * np.sqrt(accpunc[:,i,i]),
        alpha=0.2,
        color='C0'
    )
...
for i in range(3):
    ax[i].plot(tdv, np.abs(dvp[:,i] - dv[:,i]))
    ax[i].fill_between(
        tdv,
        np.zeros_like(tdv),
        2.0 * np.sqrt(dvpunc[:,i,i]),
        alpha=0.2,
        color='C0'
    )
...
for i in range(3):
    ax[i].plot(tmg, np.abs(mgp[:,i] - mg[:,i]))
    ax[i].fill_between(
        tmg,
        np.zeros_like(tmg),
        2.0 * np.sqrt(mgpunc[:,i,i]),
        alpha=0.2,
        color='C0'
    )
