...
j1 = str(diff(transpose(R2 * R1 * R0) * m, phi))
j2 = str(diff(transpose(R2 * R1 * R0) * m, theta))
j3 = str(diff(transpose(R2 * R1 * R0) * m, psi))

assert (
    j1
    == "Matrix([[0], [mx*(sin(phi)*sin(psi) + sin(theta)*cos(phi)*cos(psi)) + my*(-sin(phi)*cos(psi) + sin(psi)*sin(theta)*cos(phi)) + mz*cos(phi)*cos(theta)], [mx*(-sin(phi)*sin(theta)*cos(psi) + sin(psi)*cos(phi)) + my*(-sin(phi)*sin(psi)*sin(theta) - cos(phi)*cos(psi)) - mz*sin(phi)*cos(theta)]])"
)
assert (
    j2
    == "Matrix([[-mx*sin(theta)*cos(psi) - my*sin(psi)*sin(theta) - mz*cos(theta)], [mx*sin(phi)*cos(psi)*cos(theta) + my*sin(phi)*sin(psi)*cos(theta) - mz*sin(phi)*sin(theta)], [mx*cos(phi)*cos(psi)*cos(theta) + my*sin(psi)*cos(phi)*cos(theta) - mz*sin(theta)*cos(phi)]])"
)
assert (
    j3
    == "Matrix([[-mx*sin(psi)*cos(theta) + my*cos(psi)*cos(theta)], [mx*(-sin(phi)*sin(psi)*sin(theta) - cos(phi)*cos(psi)) + my*(sin(phi)*sin(theta)*cos(psi) - sin(psi)*cos(phi))], [mx*(sin(phi)*cos(psi) - sin(psi)*sin(theta)*cos(phi)) + my*(sin(phi)*sin(psi) + sin(theta)*cos(phi)*cos(psi))]])"
)
